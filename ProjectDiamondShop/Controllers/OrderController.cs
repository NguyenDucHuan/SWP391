    ...
    public class OrderController : Controller
    {
        ...
        private ViewOrderViewModel GetOrderDetails(string orderId)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                ...
                if (readerOrder.Read())
                {
                    orderDetails.Order = new Models.Order
                    {
        ...
        private Payment CreatePayment(APIContext apiContext, string redirectUrl)
        {
            ...
            foreach (var item in cart.Items)
            {
                itemList.items.Add(new Item()
                {
                    ...
                    currency = "USD",
                    price = item.DiamondPrice.ToString("F2"),
                    quantity = "1",
            ...
            var details = new Details()
            {
                tax = "0",
                shipping = "0",
                subtotal = totalMoney.ToString("F2")
            };
            //Final amount with details  
            var amount = new Amount()
            {
                currency = "USD",
                total = totalMoney.ToString("F2"), // Total must be equal to sum of tax, shipping and subtotal.  
                details = details
            };
